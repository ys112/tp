...
* To accommodate HR officers who deal with a large volume of applicants, executing
  `find` two or more times consecutively will lead to an `AND` search between the keywords provided between the     searches. However, this will not affect the `OR` search mentioned previously amongst the keywords provided in
